...
class TextHighlighter(object):
    ...
    def highlight(self, text, keywords):
        ...
        if(len(keywords) > 0):
            ...
            if self.max_ngram_size == 1:
                ...
                n_text = self.format_n_gram_text(
                    text, kw_list, self.max_ngram_size)
    ...
    def format_one_gram_text(self, text, relevant_words_array):
        ...
        try:
            for tk in range(len(text_tokens)):
                kw = re.sub('[!",:.;?()]$|^[!",:.;?()]|\W["!,:.;?()]',
                            '',  text_tokens[tk]).lower()
                if kw in relevant_words_array:
                    text_tokens[tk] = text_tokens[tk].lower().replace(
        ...
        while y < len(text_tokens):
            ...
            if n_gram_word_list:
                ...
                if len(n_gram_word_list[0].split(' ')) == 1:
                    ...
                    min_score_word = min(
                        kw_list, key=lambda x: relevant_words_array.index(x))
                        ...
                        if temporal_kw.lower() in relevant_words_array:
                            try:
                                if relevant_words_array.index(temporal_kw.lower()) > relevant_words_array.index(final_splited_text[-1].lower() + ' '+temporal_kw.lower()) and not re.findall(self.highlight_pre, final_splited_text[-1].lower()):
                                    term_list = [
                                        final_splited_text[-1].lower() + ' '+temporal_kw.lower()]
    ...
    def find_more_relevant(self,  y, text_tokens, n_gram, relevant_words_array, kw_list, splited_n_gram_word_list):
        ...
        for i in range(n_gram):
            ...
            k = re.sub('''[!",:.;?()]$|^[!",':.;?()]|\W["!,:.;?()]''',
                       '',  ' '.join(temporary_list[i])).lower()

            if k.lower() in relevant_words_array:
                temporary_list_two.append(k)

        n_gram_word_list = sorted(
            temporary_list_two, key=lambda x: relevant_words_array.index(x))
    ...
    def replace_token(self, text_tokens, y, n_gram_word_list):
        ...
        new_expression = txt.replace(re.sub('[!",:.;?()]$|^[!",:.;?()]|\W["!,:.;?()]',
                                            '',  txt), self.highlight_pre + n_gram_word_list[0] + self.highlight_post)
